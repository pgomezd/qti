...
%%%%%%%
% 26oct2016 - modified by P.Gomez for 3D + T data
...
elseif ndims(alpha)==4
    [nx,ny,nz,K] = size(alpha);
    Wx = Wy;
    L = nx * ny / Wx / Wy; % for distinct
    L3 = L*nz/Wz;
    ...
    alpha_LLR = zeros(Wx*Wy,nz,L,K);
    for ii=1:K
        for iz = 1:nz
            alpha_LLR(:,iz,:,ii) = im2col(alpha(:,:,iz,ii), [Wx,Wy], 'distinct');
        end
    end
    
    % get nz blocks
    alpha_LLR = reshape(alpha_LLR,[Wx*Wy*Wz,L3,K]);
    
    % threshold singular values
    s_LLR = zeros(K,L3);
    for ii=1:L3
        [UU, SS, VV] = svd(squeeze(alpha_LLR(:,ii,:)), 'econ');
        s_LLR(:,ii) = diag(SS);
        s2 = SoftThresh(s_LLR(:,ii), lambda);
        alpha_LLR(:,ii,:) = UU*diag(s2)*VV';
    end
    ...
    % reshape into image
    alpha_thresh = zeros(size(alpha));
    alpha_LLR = reshape(alpha_LLR,[Wx*Wy,nz,L,K]);  
    for ii=1:K
        for iz = 1:nz
            alpha_thresh(:,:,iz,ii) = col2im(squeeze(alpha_LLR(:,iz,:,ii)), [Wx, Wy], [nx, ny], 'distinct'); 
        end
    end
    ...
    s_vals = permute(reshape(s_LLR, [K, nx/Wx, ny / Wy, nz / Wz]), [2, 3, 4, 1]);
